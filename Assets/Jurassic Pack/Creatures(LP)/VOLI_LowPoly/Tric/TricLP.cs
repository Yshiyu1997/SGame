...
public class TricLP : Creature
{
  ...
  public AudioClip Waterflush,Hit_jaw,Hit_head,Hit_tail,Medstep,Medsplash,Sniff2,Chew,Slip,Largestep,Largesplash,Idleherb,Tric1,Tric2,Tric3,Tric4;
	...
	void FixedUpdate ()
	{
        ...
        if (UseAI)
        {
            if (Health != 0 && Vector3.Distance(this.transform.position, Manager._instance.playerObj.transform.position) <= 2)
            {
                // 开始主动攻击玩家  不能攻击情况（玩家处于无敌状态和当前玩家等级大于等于2）
                //AnmBattle(rndMove, 1, 2, 3, 4, Manager._instance.playerObj.GetComponent<Creature>());
                if (Manager._instance.playerObj.GetComponent<Creature>().Health > 0 && !Manager._instance.isInvincible && Manager._instance.level < 2)
                {
                    InitAttack(Manager._instance.playerObj.GetComponent<Creature>());
                    GameEntry.Sound.PlaySound(this.GetComponent<Enemy>().m_AttackSoundId);
                }

            }
            else
            {
                CancleAttack_ZhuDong();
            }
        }
