...
public class Creature : MonoBehaviour
{
  ...
  public MeshRenderer[] rend_1;
  public Texture[] skin, eyes;
  public enum Skin {SkinA, SkinB, SkinC,SkinD};
  ...
  public GameObject bodyCollider, headCollider, leftFootCollider, rightFootCollider;
  ...
  //Creature size
    ...
    body.mass=BaseMass*size; //creature mass based on size
        ...
        {
            boxscale = rend_1[0].bounds.extents; //bounding box scale
        }
  ...
  public void StatusUpdate()
    ...
    if(anm.GetNextAnimatorClipInfo(0).Length!=0) OnAnm=anm.GetNextAnimatorStateInfo(0);
        ...
        { 
          ...
          else { Stamina=Mathf.Clamp(Stamina-1.0f, 0.0f, 100f); Water=Mathf.Clamp(Water-1.0f, 0.0f, 100f);
        ...
        { 
          ...
          if(IsInWater) { Stamina=Mathf.Clamp(Stamina-1.0f, 0.0f, 100f);
                        //Health =Mathf.Clamp(Health-1.0f, 0.0f, 100f);
                    } 
        }

        //if(Food==0.0f | Stamina==0.0f | Water==0.0f) Health=Mathf.Clamp(Health-0.1f, 0.0f, 100f);
      ...
      if(main.TimeAfterDead==0) return;
			...
			else if(behaviorCount==-main.TimeAfterDead)
			{
				...
				if(main.creaturesList.IndexOf(main.playerObj) >= main.creaturesList.IndexOf(transform.gameObject)) { if(main.creaturesList.IndexOf(main.playerObj) > 0) main.selected--; }
  ...
  void SpawnBlood(Vector3 position)
	{
        ...
        Debug.Log("产生血"+particle);
        Destroy(particle.gameObject, 10.0f); //destroy particle
	}
  ...
  public void ManageCollision(Collision col, float pitch_max, float crouch_max, AudioSource[] source, AudioClip pain, AudioClip Hit_jaw, AudioClip Hit_head, AudioClip Hit_tail)
	{
        //Debug.Log("嘴撕咬--"+ col.transform.name+"根"+ col.transform.root.name);
        //Collided with a Creature

        //if (col.transform.root.tag.Equals("Creature"))
        if (col.transform.tag.Equals("Creature"))
        {
            ...
            if (!UseAI && OnAttack)
		  {
			  ...
			  if(other.specie==specie) { behavior="Contest"; other.behavior="Contest"; }  // 同类 竞争
			  else if(other.CanAttack) { behavior="Battle"; other.behavior="Battle"; }   // 异类可以攻击
			  else { behavior="Battle"; other.behavior="ToFlee"; }    // 一个攻击  一个逃跑
		  }
			//Eat ?
      ...
      //Attack ?
			else if(lastHit==0 && other.OnAttack)
			{
                ...
                float baseDamages = 34f; // 基础伤害
                Debug.Log("嘴撕咬"+baseDamages);
                if (col.collider.gameObject.name.StartsWith("jaw")) //bite damage   嘴撕咬
				{
                    SpawnBlood(col.GetContact(0).point);
                    //if(!IsInWater) body.AddForce(-col.GetContact(0).normal*other.body.mass/4, ForceMode.Acceleration);   //加速
                    lastHit=50;
                    if (IsDead) return;
				...
				else if(col.collider.gameObject.name.Equals("head")) //head damage   头部撞
				{
                    SpawnBlood(col.GetContact(0).point); 
				...
				else  if(!col.collider.gameObject.name.Equals("root")) //tail damage  尾巴抽打
				{
					SpawnBlood(col.GetContact(0).point);
                    if(!IsInWater) body.AddForce(col.GetContact(0).normal*other.body.mass/4, ForceMode.Acceleration);
                     ...
                     Transform RArm1=null, Transform RArm2=null, Transform RArm3=null, Transform LArm1=null, Transform LArm2=null, Transform LArm3=null, float FeetOffset=0.0f)
  {
    ...
    if(main.UseRaycast)
    {
      ...
      else if(ikType>=IkType.SmBiped) // Biped  两只脚
      {
  ...
  public void ApplyYPos()
  {
    ...
    public void Move(Vector3 dir, float force = 0, bool jump = false)
    {
        ...
        if (dir != Vector3.zero)
		{
			...
			if (!UseAI)
			{
                if (Joystick.instance.isCanControl && Manager._instance.isCanMove)
                    body.AddForce(dir * force * speed* playerSpeed, jump ? ForceMode.VelocityChange : ForceMode.Acceleration);
			}
            else
            {
...
//FLYING
  ...
  public void GetUserInputs(int idle1=0, int idle2=0, int idle3=0, int idle4=0, int eat=0, int drink=0, int sleep=0, int rise=0)
	{
        ...
        //      if (main.selected >= main.creaturesList.Count) return;
        //if(transform.gameObject==main.creaturesList[main.selected].gameObject && main.CameraMode!=0)
        //{
        if (main.creaturesList.IndexOf(main.playerObj) >= main.creaturesList.Count) return;
        if (transform.gameObject == main.creaturesList[main.creaturesList.IndexOf(main.playerObj)].gameObject && main.CameraMode != 0)
        {
            //Run key
            bool run=Input.GetKey(KeyCode.LeftShift)?true:false;  // 加速

			//Attack key
			if(CanAttack)
        {
                //if (Input.GetKey(KeyCode.Mouse0)) { behaviorCount = 500; behavior = "Hunt"; anm.SetBool("Attack", true); }  // 攻击
                //else anm.SetBool("Attack", false);
                //if (Input.GetKey(KeyCode.Space)) { behaviorCount = 500; behavior = "Hunt"; anm.SetBool("Attack", true); }  // 攻击
                //else anm.SetBool("Attack", false);
            }

            //Crouch key
            if (main.UseIK && Input.GetKey(KeyCode.LeftControl)) { crouch=Crouch_Max*size; OnCrouch=true; } // 蹲伏
            else OnCrouch=false;
            ...
            //}
            //PlayerMoveByKeyBord();
            PlayerMoveByJoystick();
            //Invert body (Ammonite & Cameroceras)  鹦鹉螺或者房角石  
            if (CanInvertBody && Input.GetKeyDown(KeyCode.R)) { if(OnInvert) OnInvert=false; else OnInvert=true; }

			//Idles key  
			if(Input.GetKey(KeyCode.E))  // 切换待机动作
			{
        ...
        switch(rndIdle)
        {
          ...
          case 4: anm.SetInteger ("Idle", idle4); break;
			...
			else if(Input.GetKey(KeyCode.F)) //Eat / Drink   吃 喝
			{
    ...
    /// 玩家移动
    /// </summary>
  public void PlayerMoveByKeyBord()
   {
    ...
    /// 玩家移动
    /// </summary>
    public void PlayerMoveByJoystick()
    {
        //if (!Manager._instance.isCanMove) return;
        bool run = Input.GetKey(KeyCode.LeftShift) ? true : false;  // 加速
        //bool run = true; // 加速
        if (!Manager._instance.isShowGameFormUI) return;
        //Move  移动
        if (Joystick.instance.rocker.transform.localPosition.x != 0 | Joystick.instance.rocker.transform.localPosition.y != 0)
        {
            //Flying/swim  水里或者天空
            if (CanSwim | (CanFly && !IsOnGround))
            {
            ...
            else
            {
                ...
                else
                {
                    //float ang = main.transform.eulerAngles.y + Mathf.Atan2(Input.GetAxis("Horizontal"), Input.GetAxis("Vertical")) * Mathf.Rad2Deg;
                    //Debug.Log("yidong1");
                    if (Joystick.instance.isCanControl)
                    {
						float ang = main.transform.eulerAngles.y + Mathf.Atan2(Joystick.instance.rocker.transform.localPosition.x, Joystick.instance.rocker.transform.localPosition.y) * Mathf.Rad2Deg;
    ...
    public void PlayerAttack()
    {
        ...
        Manager._instance.isHasAttack = true;
        Invoke("CancelPlayerAttack",0.5f);
    }
    ...
    ///  开启和关闭碰撞体 用于无敌状态
    /// </summary>
    public void PlayerColliderIsTrigger(bool isOnTrigger)
    {
        if (bodyCollider.GetComponent<CapsuleCollider>())
        {
            bodyCollider.GetComponent<CapsuleCollider>().isTrigger = isOnTrigger;
        }
        if (headCollider.GetComponent<CapsuleCollider>())
        {
            headCollider.GetComponent<CapsuleCollider>().isTrigger = isOnTrigger;
        }
        if (headCollider.GetComponent<CapsuleCollider>())
        {
            headCollider.GetComponent<CapsuleCollider>().isTrigger = isOnTrigger;
  ...
  //***********************************************************************************************************************************************************************************************************
	...
	//***********************************************************************************************************************************************************************************************************
	//FIND PREY (Find a size suited prey for carnivorous by priority)  // 食肉者的寻找合适目标
	bool FindPrey()
	{
      ...
      if(!CanSwim && other.IsInWater) continue;
			...
			else if(Food==0 && other.withersSize<withersSize*3) { objTGT=other.gameObject; V1=other.body.worldCenterOfMass; } // any prey, cannibalism allowed   可以猎杀同类 极度饥饿
		}
		if(V1==Vector3.zero) return false; else { behavior="ToHunt"; posTGT=V1; return true; }
	}

	//***********************************************************************************************************************************************************************************************************
	//FIND FRIEND (Find in current creature list a same specie creature and share his activity)   找朋友
	bool FindFriend()
	{
		foreach (GameObject o in main.creaturesList.ToArray())
		{
	...
	{                                   // 逃离
		if (loop==0 && !behavior.Equals("ToFlee") && !behavior.Equals("Battle") && !behavior.EndsWith("Hunt") && !behavior.Equals("ToTarget"))
		{
			...
			foreach (GameObject o in main.creaturesList.ToArray())
			{
                if (!o.gameObject || !transform.gameObject) continue;
  ...
  void ExecuteBehavior(int idle1=0, int idle2=0, int idle3=0, int idle4=0, int eat=0, int drink=0, int sleep=0)
    ...
    if(objCOL) // obstacle object  障碍物
    {
      ...
      if(Mathf.Abs(avoidDelta)>90) avoidDelta=0; //max avoid delta
		...
		if(objTGT) // object target  对象
    {
      other=objTGT.GetComponent<Creature>();
			if(other && !behavior.Equals("ToFlee")) posTGT=other.body.worldCenterOfMass; // 获得重心
    }

    distTGT=(transform.position-posTGT).magnitude; //target distance  目标距离
    angTGT=Quaternion.LookRotation(posTGT-transform.position); //target direction  目标方向
    delta=Mathf.DeltaAngle(angTGT.eulerAngles.y, transform.eulerAngles.y); //target angle gap  距离目标在y方向上的最短角度差  
    actionDist=(transform.position-HeadPos).magnitude; //distance from head  距离目标头部位置的距离
  ...
  enum Vect{forward, strafe, backward, zero };
    ...
    switch (type)
      ...
      case Vect.backward: anm.SetFloat("Turn", angTGT.eulerAngles.y+(delta>0.0f?180f:-180f)); break;
		  case Vect.strafe:   anm.SetFloat("Turn", angTGT.eulerAngles.y+(delta>0.0f?90:-90)); break;  // 转向低空
  ...
  void AnmHunt(int rndMove, Creature other)
    ...
    if(CanFly)
    {
      ...
      if(rndMove<25&&CanAttack) { anm.SetInteger("Move", 1); anm.SetBool("Attack", true); anm.SetInteger("Idle", 0); }
      else if(rndMove<50&&CanAttack) { anm.SetInteger("Move", -10); anm.SetBool("Attack", true); anm.SetInteger("Idle", 0); }
      else if(rndMove<75&&CanAttack) { anm.SetInteger("Move", -10); anm.SetBool("Attack", true); anm.SetInteger("Idle", 0); }
      else { anm.SetInteger("Move", -1); anm.SetBool("Attack", false); anm.SetInteger("Idle", 1); }
    }
    //Terrestrial hunt  陆地捕猎
    else if(!CanSwim | (CanSwim && CanWalk && !IsInWater))
    {
      if(objCOL==objTGT&&CanAttack) { anm.SetBool("Attack", true);  AnmTurn(Vect.zero); }
      else if(distTGT<actionDist) { anm.SetInteger("Move", 0);  anm.SetBool("Attack", false);  AnmTurn(Vect.zero); }
      else if(distTGT<actionDist*1.25f&&CanAttack) { anm.SetInteger("Move",  rndMove<50?1:2);  anm.SetBool("Attack", true); AnmTurn(Vect.forward); }
    ...
    else
    {
      ...
      else if((distTGT<actionDist*1.25f | objCOL==objTGT)&&CanAttack) { anm.SetInteger("Move",  rndMove<50?1:2);  anm.SetBool("Attack", true); AnmTurn(Vect.forward); }
      else { anm.SetInteger("Move", aim?0:2); anm.SetBool("Attack", false); AnmTurn(Vect.forward); }
    }
	}
    /// <summary>
    ///  主动攻击
    /// </summary>
    public void InitAttack(Creature other)
    {
        //transform.LookAt(other.gameObject.transform);
        //lookTGT = other.gameObject.transform.position;
        objTGT = other.gameObject;
        anm.SetInteger("Move", rndMove < 25 ? 1 : 2);
        AnmTurn(Vect.zero); anm.SetBool("Attack", true);
    }
    /// <summary>
    ///  取消主动攻击
    /// </summary>
    public void CancleAttack_ZhuDong()
    {
        anm.SetBool("Attack", false);
    }
   public  void AnmBattle(int rndMove, int idle1, int idle2, int idle3, int idle4, Creature other)
	{
    //Air battles
    if(CanFly)
    {
      ...
      if(rndMove<75)
      {
        ...
        if((objCOL==objTGT | distTGT<actionDist*1.25f)&&CanAttack) anm.SetBool("Attack", true); else anm.SetBool("Attack", false);
    ...
    else if(!CanSwim | (CanSwim && CanWalk && !IsInWater))
    {
      if((other&&((rndMove<50 | other.rndMove<50) && distTGT<actionDist*2.0f)) | (!other&&(rndMove<50 && distTGT<actionDist*2.0f)) )
      {
        ...
        else if(distTGT<actionDist*1.25f&&CanAttack) { anm.SetInteger("Move", rndMove<25?0:1); AnmTurn(Vect.forward); anm.SetBool("Attack", true); }
        else if(CanAttack){ anm.SetInteger("Move", rndMove<25?1:2);  AnmTurn(Vect.forward); anm.SetBool("Attack", true); }
      }
      else if(distTGT<actionDist*5.0f)
      {        
                Debug.Log("跑");
    ...
    else if(CanSwim)
    {
      ...
      {
        AnmTurn(Vect.forward);  
        if(distTGT<actionDist*2.0f&&CanAttack) { anm.SetInteger("Move", rndMove<50?0:1);  anm.SetBool("Attack", true); }
        else if(distTGT<actionDist*3.0f&&CanAttack) { anm.SetInteger("Move", 2);  anm.SetBool("Attack", true); }
